...
interface AuthApi {
    ...
    @POST("start/register")
    fun signUp(@Field("User[login]") login: String, @Field("User[firstname]") firstname: String, @Field("User[lastname]") lastname: String, @Field("User[birthday]") birthday: String, @Field("User[password]") password: String, @Field("User[email]") email: String): Observable<RootObject>

    @POST("start/login/")
    fun signIn(@Body user: UserLogIn): Observable<RootObject>
}